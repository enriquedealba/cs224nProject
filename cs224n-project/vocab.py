...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        chars_padded = pad_sents_char(char_ids, self.char2id['<pad>'])
        chars_var = torch.tensor(chars_padded, dtype=torch.long, device=device)
        chars_var = chars_var.permute(1, 0, 2) ## double check this!!!
        return chars_var.contiguous()
